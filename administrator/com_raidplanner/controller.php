...
class RaidPlannerController extends JController
{
	...
	function display()
	{
		...
		if ($this->getTask() == 'service') {
			...
			$query = "SELECT p.profile_id,p.group_id FROM #__raidplanner_profile AS p LEFT JOIN #__raidplanner_groups AS g ON g.group_id = p.group_id LEFT JOIN #__users AS u ON u.id = p.profile_id WHERE (g.group_name IS NULL OR u.name IS NULL OR g.group_name='Guest')"; 
			$db->setQuery($query);
			$list = $db->loadObjectList();
			if (count($list) > 0)
			{
